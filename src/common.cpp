...
loadImageDirectory(const std::string &path) {
  ...
  do {
    ...
    auto img_data = skity::Data::MakeFromFileName(fullName.c_str());
...
std::vector<std::string> file_dialog(
  ...
  int size =
      MultiByteToWideChar(CP_UTF8, 0, &filter[0], (int)filter.size(), NULL, 0);
  std::wstring wfilter(size, 0);
  MultiByteToWideChar(CP_UTF8, 0, &filter[0], (int)filter.size(), &wfilter[0],
                      size);
  ...
  while (tmp[i] != '\0') {
    ...
    if (tmpSize > 0) {
      int filenameSize = WideCharToMultiByte(CP_UTF8, 0, &tmp[i], tmpSize, NULL,
                                             0, NULL, NULL);
      filename.resize(filenameSize, 0);
      WideCharToMultiByte(CP_UTF8, 0, &tmp[i], tmpSize, &filename[0],
                          filenameSize, NULL, NULL);
    }
  ...
  // The safest separator for multiple selected paths is /, since / can never
  // occur in file names. Only where two paths are concatenated will there be
  // two / following each other.
  if (multiple) cmd += "--multiple --separator=\"/\" ";
  if (save) cmd += "--save ";
  cmd += "--file-filter=\"";
