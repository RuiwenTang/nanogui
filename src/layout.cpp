...
Vector2i BoxLayout::preferredSize(const Widget *widget) const {
  ...
  for (auto w : widget->children()) {
    ...
    Vector2i ps = w->preferredSize(), fs = w->fixedSize();
  ...
  for (auto w : widget->children()) {
    ...
    switch (mAlignment) {
      ...
      case Alignment::Fill:
        pos[axis2] += mMargin;
        targetSize[axis2] =
            fs[axis2] ? fs[axis2] : (containerSize[axis2] - mMargin * 2);
        break;
    }
...
Vector2i GroupLayout::preferredSize(const Widget *widget) const {
  ...
  for (auto c : widget->children()) {
    ...
    Vector2i ps = c->preferredSize(), fs = c->fixedSize();
    Vector2i targetSize(fs[0] ? fs[0] : ps[0], fs[1] ? fs[1] : ps[1]);

    bool indentCur = indent && label == nullptr;
    height += targetSize.y();
    width = std::max(
        width, targetSize.x() + 2 * mMargin + (indentCur ? mGroupIndent : 0));
...
void GroupLayout::performLayout(Widget *widget) const {
  int height = mMargin,
      availableWidth =
...
Vector2i GridLayout::preferredSize(const Widget *widget) const {
  /* Compute minimum row / column sizes */
  std::vector<int> grid[2];
  computeLayout(widget, grid);
...
void GridLayout::computeLayout(const Widget *widget,
...
void GridLayout::performLayout(Widget *widget) const {
  Vector2i fs_w = widget->fixedSize();
  Vector2i containerSize(fs_w[0] ? fs_w[0] : widget->width(),
                         fs_w[1] ? fs_w[1] : widget->height());
  ...
  for (int i = 0; i < 2; i++) {
    ...
    if (gridSize < containerSize[i]) {
      ...
      for (int j = 0; rest > 0 && j < dim[i]; --rest, ++j) grid[i][j] += 1;
    }
  }
...
Vector2i AdvancedGridLayout::preferredSize(const Widget *widget) const {
  /* Compute minimum row / column sizes */
  std::vector<int> grid[2];
  computeLayout(widget, grid);
...
void AdvancedGridLayout::performLayout(Widget *widget) const {
  std::vector<int> grid[2];
  computeLayout(widget, grid);
  ...
  for (int axis = 0; axis < 2; ++axis) {
    ...
    for (Widget *w : widget->children()) {
      ...
      int ps = w->preferredSize()[axis], fs = w->fixedSize()[axis];
...
void AdvancedGridLayout::computeLayout(const Widget *widget,
  ...
  for (int axis = 0; axis < 2; ++axis) {
    ...
    for (int phase = 0; phase < 2; ++phase) {
      for (auto pair : mAnchor) {
        ...
        for (int i = anchor.pos[axis]; i < anchor.pos[axis] + anchor.size[axis];
             ++i) {
