...
#endif
#pragma warning( \
    disable : 4251)  // warning C4251: class X needs to have dll-interface to be
...
/// Cursor shapes available to use in GLFW.  Shape of actual cursor determined
/// by Operating System.
enum class Cursor {
  ...
  CursorCount  ///< Not a cursor --- should always be last: enables a loop over
               ///< the cursor types.
};
 ...
 * convenient methods and terminology for thinking of it as a color.  The data
 * operates in the same way as ``Eigen::Vector4f``, and the following values are
 * identical:
 *
 * \rst
 * +---------+-------------+-----------------------+-------------+
 ...
 */
extern NANOGUI_EXPORT std::vector<std::string> file_dialog(
    const std::vector<std::pair<std::string, std::string>> &filetypes,
