...
class NANOGUI_EXPORT ImageView : public Widget {
 public:
  ImageView(Widget* parent, std::shared_ptr<skity::Pixmap> image);
  ~ImageView();
 ...
 private:
  ...
  void drawWidgetBorder(skity::Canvas* canvas) const;
  void drawImageBorder(skity::Canvas* canvas) const;
  void drawHelpers(skity::Canvas* canvas) const;
  static void drawPixelGrid(skity::Canvas* canvas,
                            const Vector2f& upperLeftCorner,
                            const Vector2f& lowerRightCorner, float stride);
  void drawPixelInfo(skity::Canvas* canvas, float stride) const;
  void writePixelInfo(skity::Canvas* canvas, const Vector2f& cellPosition,
                      const Vector2i& pixel, float stride,
                      float fontSize) const;

  // Image parameters.
  std::shared_ptr<skity::Pixmap> mImage;
  Vector2i mImageSize;
