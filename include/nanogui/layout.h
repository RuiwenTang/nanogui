...
public:
  ...
  GridLayout(Orientation orientation = Orientation::Horizontal,
             int resolution = 2, Alignment alignment = Alignment::Middle,
             int margin = 0, int spacing = 0)
  ...
  /// Sets the number of rows or columns (depending on the Orientation) of this
  /// GridLayout.
  void setResolution(int resolution) { mResolution = resolution; }

  /// The spacing at the specified axis (row or column number, depending on the
  /// Orientation).
...
protected:
  ...
  /// The number of rows or columns before starting a new one, depending on the
  /// Orientation.
  int mResolution;
 ...
 *    // Add a centered label at grid position (1, 5), which spans two
  ...
  Anchor anchor(const Widget *widget) const {
    ...
    if (it == mAnchor.end())
      throw std::runtime_error(
          "Widget was not registered with the grid layout!");
    return it->second;
  }
