...
class NANOGUI_EXPORT TabHeader : public Widget {
 public:
  ...
  /**
   * Returns a pair of Vectors describing the top left (pair.first) and the
   * bottom right (pair.second) positions of the rectangle containing the active
   * tab button. Returns two zero vectors if the active button is not visible.
   */
 ...
 private:
  ...
  class TabButton {
   public:
    ...
    void calculateVisibleString();
    void drawAtPosition(skity::Canvas* canvas, const Vector2i& position,
                        bool active);
    void drawActiveBorderAt(skity::Canvas* canvas, const Vector2i& position,
                            float offset, const Color& color);
    void drawInactiveBorderAt(skity::Canvas* canvas, const Vector2i& position,
                              float offset, const Color& color);

    void rebuildLabelBlob();

   private:
    TabHeader* mHeader;
    std::string mLabel;
    std::shared_ptr<skity::TextBlob> mLabelBlob;
    std::shared_ptr<skity::TextBlob> mVisibleBlob;
    Vector2i mSize;
  ...
  void drawControls(skity::Canvas* canvas);
  ...
  skity::Paint mIconPaint;
  std::shared_ptr<skity::TextBlob> mIconLeftBlob;
  std::shared_ptr<skity::TextBlob> mIconRightBlob;
