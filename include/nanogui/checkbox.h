...
class NANOGUI_EXPORT CheckBox : public Widget {
 public:
   ...
   */
  CheckBox(
      Widget *parent, const std::string &caption = "Untitled",
   ...
   * well as released, this function sets \ref nanogui::CheckBox::mPushed to
   * ``true`` when parameter ``down == true``.  When the second event (``down ==
   * false``) is fired, \ref nanogui::CheckBox::mChecked is inverted and \ref
   * nanogui::CheckBox::mCallback is called.
   *
   * That is, the callback provided is only called when the mouse button is
   * released,
   * **and** the click location remains within the CheckBox boundaries.  If the
   * user clicks on the CheckBox and releases away from the bounds of the
   * CheckBox, \ref nanogui::CheckBox::mPushed is simply set back to ``false``.
   */
 ...
 private:
  skity::TextBlobBuilder blobBuilder;
