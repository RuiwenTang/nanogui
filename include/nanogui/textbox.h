...
class NANOGUI_EXPORT TextBox : public Widget {
 public:
  ...
  std::shared_ptr<skity::Pixmap> unitsImage() const { return mUnitsImage; }
  void setUnitsImage(std::shared_ptr<skity::Pixmap> image) {
    mUnitsImage = image;
  }
  ...
  void setCallback(
      const std::function<bool(const std::string &str)> &callback) {
    mCallback = callback;
  }
 ...
 protected:
  ...
  std::string mUnits;
  std::string mFormat;
  std::shared_ptr<skity::Pixmap> mUnitsImage;
...
class IntBox : public TextBox {
 public:
  ...
  void setMinValue(Scalar minValue) { mMinValue = minValue; }
  void setMaxValue(Scalar maxValue) { mMaxValue = maxValue; }
  ...
  virtual bool mouseDragEvent(const Vector2i &p, const Vector2i &rel,
    ...
    if (mSpinnable && !focused() &&
        button == 2 /* 1 << GLFW_MOUSE_BUTTON_2 */ && mMouseDownPos.x() != -1) {
      int valueDelta =
          static_cast<int>((p.x() - mMouseDownPos.x()) / float(10));
